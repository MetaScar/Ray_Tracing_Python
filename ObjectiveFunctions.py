...
def focusObjective(cpRays, z, x):
    total_S = 0 # variable to keep track of total power density, scaled by the distance to the focal point.

    for i in range(len(cpRays)):
        d2 = getClosestPosition(cpRays[i], z, x)
        total_S = total_S + cpRays[i].PoyntingMag*tf.math.exp(-4*d2)
        
    return total_S

# Given a ray, this helper function finds the ray position that is closest to the specified focal point.
# It then returns the distance to that focal point squared.
