...
def OrdinaryWavePropagation(rx_init, ry_init, rz_init, pox_init, poy_init, poz_init, a0, a1, a2, time_step, E):

    h = time_step; # time step
    ko = 209.4395102 # For f = 10 GHz

    # Calculate the derivatives of the index of refraction:
    e_perp, deperp_dx, deperp_dy, deperp_dz = hp.getOrdinaryIndex(rx_init, ry_init, rz_init, a0, a1, a2)
...
def ExtraordinaryWavePropagation(rx_init, ry_init, rz_init, pex_init, pey_init, pez_init, a0, a1, a2, b0, b1, b2, c0, c1, c2, time_step, E):

    ko = 209.4395102 # For f = 10 GHz

    # Calculates e_perp, e_paralell, and the associated spatial derivatives:
    e_perp, deperp_dx, deperp_dy, deperp_dz = hp.getOrdinaryIndex(rx_init, ry_init, rz_init, a0, a1, a2)
    e_para, depara_dx, depara_dy, depara_dz = hp.getExtraordinaryIndex(rx_init, ry_init, rz_init, b0, b1, b2)
    ...
    k1 = -2*(e_para - e_perp)*tf.tensordot([pex_init, pey_init, pez_init], d_local, axes=1)*(pex_init*ddx_x + pey_init*ddy_x + pez_init*ddz_x) - e_perp*depara_dx - (e_para + (tf.norm([pex_init, pey_init, pez_init])**2))*deperp_dx
    k2 = -2*(e_para - e_perp)*tf.tensordot([pex_init, pey_init, pez_init], d_local, axes=1)*(pex_init*ddx_y + pey_init*ddy_y + pez_init*ddz_y) - e_perp*depara_dy - (e_para + (tf.norm([pex_init, pey_init, pez_init])**2))*deperp_dy
    k3 = -2*(e_para - e_perp)*tf.tensordot([pex_init, pey_init, pez_init], d_local, axes=1)*(pex_init*ddx_z + pey_init*ddy_z + pez_init*ddz_z) - e_perp*depara_dz - (e_para + (tf.norm([pex_init, pey_init, pez_init])**2))*deperp_dz
