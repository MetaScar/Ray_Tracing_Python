...
# This ray-tracing algorithm performs all tracing of rays in parallel to leverage GPU acceleration.
# This version of the code supports ISOTROPIC MATERIALS ONLY.
# THIS IS THE MOST UP-TO-DATE VERSION OF THE CODE.

### ---------- USER INPUT STARTS HERE ---------- ###

# This debugging step checks for nans or infinities in the code.
...
with tf.GradientTape() as tape:
    ...
    materialCoefs_lens = tf.Variable([A1, A2, C1, C2, emax])
...
gradient = tape.gradient(loss, materialCoefs_lens)
...
# aperture_positions = nh.calcAperturePositions(positions_final, wave_vectors_final, material_IDs, focal_plane=6.0, group_IDs=group_IDs, groupID=1)
...
radius = lens_radius
height = lens_height
z_min = -0.5*lens_height

# Specific group of rays to be plotted:
specificGroupID = 1

# Axis limits:
xy_max = lens_radius + r_padding/2 # xmin = -xy_max, xmax = xy_max, ymin = -xy_max, ymax = xy_max
z_max = lens_height + z_padding # zmin = -z_max, zmax = z_max

# Plotting:
pf.plotRaysAndCyl(radius, height, z_min, Num_rays_active, step, positions_final, group_IDs, specificGroupID, xy_max, z_max)